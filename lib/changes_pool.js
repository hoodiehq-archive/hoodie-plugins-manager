...
exports.create = function (couch_url, callback) {
    db_updates.available(couch_url, function (err, exists) {
        ...
        var pool;
        if (exists) {
            pool = exports.createDbUpdatesPool(couch_url);
        }
        else {
            pool = exports.createLegacyPool(couch_url);
        }
...
exports.createLegacyPool = function (couch_url) {
    console.log('WARNING: using legacy changes feed pooling (couchdb < 1.4)');
    var feeds = {};
    ...
    var queue = async.queue(function (task, callback) {
        ...
        couchr.get(db_url + '/_all_docs', q, function (err, body, res) {
            ...
            body.rows.forEach(task.handler);
            callback();
        });
    }, 1);
